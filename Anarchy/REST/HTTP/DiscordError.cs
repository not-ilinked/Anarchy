namespace Discord
{
    public enum DiscordError
    {
        MessageOnlyError,
        UnknownAccount = 10001,
        UnknownApp,
        UnknownChannel,
        UnknownGuild,
        UnknownIntegration,
        UnknownInvite,
        UnknownMember,
        UnknownMessage,
        UnknownOverwrite,
        UnknownProvider,
        UnknownRole,
        UnknownAuthAppToken,
        UnknownUser,
        UnknownEmoji,
        UnknownWebhook,
        UnknownSession = 10020,
        UnknownGiftCode = 10038,
        UnknownGuildSubscription = 10050,
        UnknownInteraction = 10062,
        UserOnly = 20001,
        BotOnly,
        MaximumGuilds = 30001,
        MaximumFriends,
        MaximumPins,
        MaximumRoles,
        MaximumEmojis = 30008,
        MaximumConnections,
        MaximumReactions = 30010,
        MaximumGuildChannels = 30013,
        MaximumGuildMembers = 30019,
        MaximumGuildSubscriptions = 30026,
        Unauthorized = 40001,
        AccountUnverified,
        OpeningDirectMessagesTooFast,
        UserIsBanned = 40007,
        AccountOwnsGuilds = 40011,
        NotConnectedToVoice = 40032,
        MessageAlreadyCrossposted,
        MissingAccess = 50001,
        InvalidAccountType,
        CannotExecuteInDM,
        WidgetDisabled,
        CannotEditMessageByOtherUser,
        CannotSendEmptyMessage,
        CannotDMThisUser,
        CannotSendMessagesInVC,
        ChannelVerificationTooHigh,
        OAuth2AppDoesNotHaveABot,
        OAuth2AppLimitReached,
        InvalidOAuthState,
        MissingPermissions,
        InvalidAuthToken,
        NoteTooLong,
        InvalidBulkDeleteAmount,
        PasswordDoesNotMatch = 50018,
        CannotPinMessageInOtherChannel = 50019,
        InvalidPhoneNumber = 50022,
        InvalidActionForChannel = 50024,
        CannotExecuteOnSysMessage,
        InvalidRecipient = 50033,
        InvalidOAuthAccessToken,
        InvalidFormBody,
        InvalidAPIVersion = 50041,
        InvalidPaymentMethod = 50048,
        NitroGiftRedeemed = 50050,
        InvalidGuild = 50055,
        GuildSubscriptionCooldown = 50069,
        IncomingFriendRequestsDisabled = 80000,
        InvalidDiscriminator = 80004,
        ReactionBlocked = 90001,
        AlreadySubscribed = 100004,
        PaymentMethodRequired = 100006,
        PendingPurchaseRequest = 100016,
        PaymentRequied = 100018,
        AdditionalPlansMustMatchSubscriptionInterval = 10028,
        PaymentRequiresAuthentication = 100029,
        ResourceOverloaded = 130000,
        StageAlreadyOpen = 150006,
        CannotReplyNoMessageHistoryPermission = 160002,
        ThreadAlreadyCreatedForMessage = 160004,
        ThreadLocked,
        MaximumActiveThreadsReached,
        MaximumActiveAnnouncementsReached,
        InvalidJSONLottieFile = 170001,
        LottiesCannotContainRasterizedImages,
        StickerMaximumFramerateExceeded,
        StickerMaximumFramesThousand,
        LottieAnimationMaximumDimensionsExceeded,
        StickerFramerateTooSmallOrLarge,
        StickerDurationExceedsMaxFiveSeconds,
        CannotUpdateFinishedUpdate = 180000,
        FailedToCreateStageForNeededStage = 180002,
        MessageBlockedByAutoMod = 200000,
        TitleBlockedByAutoMod,
        WebhookNeedsThreadNameOrThreadID = 220001,
        WebhookCannotHaveBothThreadNameAndThreadID,
        WebhooksCanOnlyCreateThreadsInForumChannel,
        WebhookServicesCannotBeUsedInForumChannels,
        MessageBlockedByHarmfulLinksFilter = 240000,
        CannotEnableOnboardingRequirementsNotMet = 350000,
        CannotUpdateOnboardingWhileBelowRequirements
    }
}
