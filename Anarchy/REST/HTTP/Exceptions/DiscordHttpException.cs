...
namespace Discord
{
    public class DiscordHttpException : Exception
    {
        ...
        {
            FieldErrorDictionary dict = new FieldErrorDictionary();

            foreach (JProperty child in obj.Children())
            {
                if (child.Name == "_errors")
                {
                    dict.Errors = child.Value.ToObject<List<DiscordFieldError>>();
                }
                else
                {
                    dict[child.Name] = FindErrors((JObject)child.Value);
                }
            }
