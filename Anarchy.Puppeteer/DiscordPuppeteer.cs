...
namespace Discord
{
    public static class DiscordPuppeteer
    {
        ...
        public static async Task StartAsync()
        {
            ...
            Browser browser = await Puppeteer.LaunchAsync(new LaunchOptions { Headless = true, IgnoreHTTPSErrors = true, Args = new string[] { "--no-sandbox", "--disable-web-security" } });
        ...
        public static async Task<JToken> CallAsync(DiscordClient parentClient, string method, string url, object data = null)
        {
            ...
            if (method == "POST")
            {
                ...
                if (url.Contains("/invites/"))
                {
                    ...
                    GuildInvite invite = await CallAsync<GuildInvite>(parentClient, "GET", parentClient.HttpClient.BaseUrl + "/invites/" + invCode + $"?inputValue={invCode}&with_counts=true&with_expiration=true");
...
";
            ...
            DiscordResponse result = await _page.EvaluateFunctionAsync<DiscordResponse>(expression);

            if (result.Status >= 400)
            {
                if (result.Status == 429)
                {
                    throw new RateLimitException(result.Body.Value<int>("retry_after"));
                }
                else
                {
                    throw new DiscordHttpException(result.Body.ToObject<DiscordHttpError>());
                }
            }
        ...
        {
            return (await CallAsync(parentClient, method, url, data)).ToObject<T>();
        }
