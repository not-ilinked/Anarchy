...
namespace MusicBot
{
    internal class Program
    {
        ...
        public static bool CanModifyList(DiscordSocketClient client, DiscordMessage message)
        {
            ...
            else if (!client.GetVoiceStates(message.Author.User.Id).GuildVoiceStates.TryGetValue(message.Guild.Id, out DiscordVoiceState state) || state.Channel == null || state.Channel.Id != voiceClient.Channel.Id)
            {
                message.Channel.SendMessage("You must be connected to the same voice channel as me to skip songs");
            }
            else if (!TrackLists.TryGetValue(message.Guild.Id, out TrackQueue queue) || queue.Tracks.Count == 0)
            {
        ...
        {
            if (TrackLists.TryGetValue(args.Client.Guild.Id, out TrackQueue list) && !list.Running)
            {
                list.Start();
            }
        }
