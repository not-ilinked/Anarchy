...
namespace GuildDuplicator
{
    internal class Program
    {
        ...
        private static void Client_OnLoggedIn(DiscordSocketClient client, LoginEventArgs args)
        {
            ...
            try
            {
                SocketGuild targetGuild = client.GetCachedGuild(guildId);

                Console.WriteLine("Creating guild...");
                DiscordGuild guild = client.CreateGuild(targetGuild.Name, targetGuild.Icon == null ? null : targetGuild.Icon.Download(), targetGuild.Region);

                Console.WriteLine("Creating roles...");
                Dictionary<ulong, ulong> dupedRoles = new Dictionary<ulong, ulong>();
                foreach (DiscordRole role in targetGuild.Roles.OrderBy(r => r.Position).Reverse())
                {
                    RoleProperties properties = new RoleProperties() { Name = role.Name, Color = role.Color, Mentionable = role.Mentionable, Permissions = role.Permissions, Seperated = role.Seperated };
                    ...
                    {
                        dupedRoles[role.Id] = guild.CreateRole(new RoleProperties() { Name = role.Name, Color = role.Color, Mentionable = role.Mentionable, Permissions = role.Permissions, Seperated = role.Seperated }).Id;
                    }
                }

                Console.WriteLine("Creating emojis...");
                Dictionary<ulong, ulong> dupedEmojis = new Dictionary<ulong, ulong>();
                foreach (DiscordEmoji emoji in targetGuild.Emojis)
                {
                    try
                    {
                        DiscordEmoji created = guild.CreateEmoji(new EmojiProperties() { Name = emoji.Name, Image = emoji.Icon.Download() });
                ...
                foreach (GuildChannel channel in targetGuild.Channels.OrderBy(c => c.Type != ChannelType.Category))
                {
                    GuildChannel ourChannel = guild.CreateChannel(channel.Name, TranslateChannelType(channel), channel.ParentId.HasValue ? dupedChannels[channel.ParentId.Value] : (ulong?)null);
                    ourChannel.Modify(new GuildChannelProperties() { Position = channel.Position });

                    if (ourChannel.Type == ChannelType.Text)
                    {
                        TextChannel channelAsText = (TextChannel)channel;
                    ...
                    {
                        VoiceChannel channelAsVoice = (VoiceChannel)channel;
                    ...
                    foreach (DiscordPermissionOverwrite overwrite in channel.PermissionOverwrites)
                    {
                ...
                bool hasWelcomeScreen = TryGetWelcomeScreen(targetGuild, out WelcomeScreen welcomeScreen);
                bool hasVerificationForm = TryGetVerificationForm(targetGuild, out GuildVerificationForm verificationForm);
                ...
                if (hasWelcomeScreen || hasVerificationForm)
                {
                    ...
                    if (hasWelcomeScreen)
                    {
                        ...
                        foreach (WelcomeChannel channel in welcomeScreen.Channels)
                        {
                            ulong? emojiId = null;
                    ...
                    foreach (GuildChannel channel in targetGuild.Channels)
                    {
        ...
        private static bool TryGetInvite(SocketGuild guild, out string code)
        {
            ...
            {
                foreach (GuildChannel channel in guild.Channels)
                {
